  ...
  <div class="bghhP"> 
    <div v-for="list,i in lists" :key="i">
      ...
      <v-card outlined class="m-2">
        <v-card-text >
            <div class="w-full">
                <div class="flex" v-if="list.course_data">
                    <v-avatar size="90">
                        <v-img :src="$url+list.course_data.image_1"></v-img>
                    </v-avatar> 
                     <div class="ml-2" v-if="list.course_data">
                        <h3 v-if="list.course_data" class="text-black text-base font-semibold mb-0">{{ list.course_data.name}}  </h3> 
                        <div> 
                            <p style="font-size:14px;">ลงทะเบียนเมื่อ {{$core.convertDate(list.created_at)}}</p>
                        </div>
                    </div>  
                </div>
            </div>
      ...
      <!-- <v-card outlined class="m-2">
        <v-card-text>
          <h2 class="font-semibold text-black">{{list.tier_name}} ({{ (list.amount)? Number(list.amount).toLocaleString() : 0}}.-)</h2>
          <h2 class="font-semibold">ซื้อเมื่อ {{$core.convertDate(list.created_at)}}</h2>
          <p>{{list.ect}}</p>
...
export default {
  ...
  methods:{
    async run(){
      let lists = await User.getRegister(this.user.id) 
      lists = _.orderBy(lists.list, ['created_at'], ['desc'])
      this.lists = lists
    },
  }
}
