    ...
    <div class="mb-10 account-bg">
      <div class="flex flex-col" v-if="response">
        <v-tabs v-model="tab">
          ...
          <v-tab-item>
            <div class="account-bg h-screen">
              <div class="w-full p-6">
                <center>
                  <v-img
                    v-if="user.image"
                    class="rounded-full h-40 w-40 mt-6"
                    :src="`${$url}/${user.image}`"
                    alt=""
                  ></v-img>
                  <img
                    v-else
                    class="rounded-full h-40 w-40 mt-6"
                    src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTyHnAAxZw7h-1IEUa5LF2MmLteCLb46LkypKTf1xI&s"
                    alt=""
                  />
                </center>
                <v-file-input
                ...
                <v-form class="w-full mt-6">
                  <h3 class="font-semibold mb-2">
                    รหัสสมาชิก: {{ user.fitness_id }}
                  </h3>
                  <v-text-field
                    readonly
                    v-model="user.username"
                    label="เบอร์โทร (สำหรับเข้าสู่ระบบ)"
                  ></v-text-field>
                  <v-text-field
                  ...
                  ></v-text-field>
                  <v-btn @click="updateProfile()" block color="success"
                    >บันทึกข้อมูล</v-btn
                  >
                </v-form>
                <br />
                ...
                <v-btn block outlined @click="removeAccount()" color="error">{{
                  $l("ลบบัญชีผู้ใช้", "Remove Account")
                }}</v-btn>
              </div>
            </div>
          </v-tab-item>
          ...
          <v-tab-item>
            <div class="w-full h-screen p-6 account-bg">
          ...
          <v-tab-item>
            <div class="w-full h-screen p-6 account-bg">
              <v-form ref="formPassword">
                ...
                ></v-text-field>
                <v-btn @click="changePassword()" depressed block color="info">{{
                  $l("เปลี่ยนรหัสผ่าน", "Change Password")
                }}</v-btn>
              </v-form>
            </div>
          </v-tab-item>
  ...
  methods: {
    ...
    async updateProfile() {
      ...
      if (check) {
        ...
        let user = await Core.putHttp(
          `/api/account/userprofile/${this.user.id}/`,
          this.form
        );
        if (user.id) {
          await Auth.setUser();
    ...
    async uploadProfile() {
      ...
      if (check) {
        ...
        let user = await Core.putImageHttp(
          `/api/account/userprofile/${this.user.id}/`,
          form
        );
        if (user.id) {
    ...
    async changePassword() {
      ...
      if (validate) {
        let check = await Web.confirm(
          this.$l("ยืนยันการแก้ไขรหัสผ่าน", "Are you sure to change password?")
        );
        if (check) {
          this.response = false;
          let change = await Core.postHttp(
            `/api/auth/v2/change-password/`,
            this.formPassword
          );
          if (change.old_password) {
            await Web.alert(
              this.$l("รหัสผ่านเดิมไม่ถูกต้อง", "Old Password is incorrect."),
              `error`
            );
          } else if (
            this.formPassword.password != this.formPassword.password_confirm
          ) {
            await Web.alert(
              this.$l("รหัสผ่านใหม่ไม่ตรงกัน", "Password is not match."),
              "error"
            );
          } else if (change.detail) {
            await Web.alert(
              this.$l("แก้ไขข้อมูลสำเร็จ", "Change Password Success"),
              "success"
            );
          }
          this.formPassword = {};
    ...
    async removeAccount() {
      let check = await this.$web.confirm(
        this.$l("ยืนยันการลบบัญชีผู้ใช้", "Are you sure to delete account?"),
        this.$l(
          "เมื่อลบบัญชีผู้ใช้แล้วจะไม่สามารถกู้คืนได้ และข้อมูลทั้งหมดจะถูกลบออกจากระบบ",
          "When you delete account, you can't restore it. All data will be deleted from the system."
        )
      );
      if (check) {
        this.response = false;
    ...
    async sumAge() {
      this.response = false;
      this.form.age = Number(
        this.$2date(this.form.birth_date, moment(), "years")
      );
